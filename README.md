...
In the [coding section provided](https://github.com/lerrylei/arduino-robohand/blob/main/glove_controlled_robotic_hand.ino), its important to realize that parameters will be different depending on each flex sensor and servo motor. In order to organize values clearly, I included the `flexTest()` function to test values for the flex sensors.
